...
class httpApiWorker {
  ...
  async startServer () {
    console.log('inside ~function=startServer. STARTING http_api WORKER')
    const vm = this
  ...
  runExpressServer () {
    console.log('info inside ~function=runExpressServer.')
    const vm = this
    vm.app = express()
    vm.app.use(helmet({
      noCache: true
    ...
    vm.app.use((err, req, res, next) => {
      ...
      if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
        console.error('Error while sending request (JSON invalid)', err)
        return res.sendJson({
